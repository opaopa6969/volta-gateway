...
pub fn build_router(state: AppState) -> Router {
    ...
    let oidc_callback_routes = Router::new()
        ...
        .route(
            ...
            post(handlers::oidc::callback_complete),
        )
    ...
    let passkey_routes = Router::new()
        .route(
            "/auth/passkey/start",
            post(handlers::passkey_flow::auth_start),
        )
        .route(
            "/auth/passkey/finish",
            post(handlers::passkey_flow::auth_finish),
        )
        // Discoverable-credential (username-less) passkey flow (#58)
        .route(
            "/auth/passkey/discover/start",
            post(handlers::passkey_flow::discover_start),
        )
        .route(
            "/auth/passkey/discover/finish",
            post(handlers::passkey_flow::discover_finish),
        )
    ...
    let invite_routes = Router::new()
        .route(
            "/invite/{code}/accept",
            post(handlers::manage::accept_invite),
        )
    ...
    let registration_routes = Router::new()
        ...
        .route(
            ...
            post(handlers::registration::register_verify_email),
        )
        .route(
            "/auth/register/resend-verification",
            post(handlers::registration::register_resend),
        )
    ...
    let device_routes = Router::new()
        .route(
            ...
            post(handlers::device::device_authorization),
        )
    ...
    Router::new()
        ...
        .route(
            "/api/me/sessions",
            delete(handlers::session::revoke_all_sessions),
        )
        .route(
            "/api/me/sessions/{id}",
        ...
        // MFA
        .route(
            "/api/v1/users/{userId}/mfa/totp/setup",
            post(handlers::mfa::totp_setup),
        )
        .route(
            "/api/v1/users/{userId}/mfa/totp/verify",
            post(handlers::mfa::totp_verify_setup),
        )
        .route(
            "/api/v1/users/{userId}/mfa/totp",
            delete(handlers::mfa::totp_disable),
        )
        .route("/api/v1/users/me/mfa", get(handlers::mfa::mfa_status))
        .route(
            "/api/v1/users/{userId}/mfa/recovery-codes/regenerate",
        ...
        .route(
            ...
            post(handlers::signing_key::rotate_key),
        )
        .route(
            "/api/v1/admin/keys/{kid}/revoke",
            post(handlers::signing_key::revoke_key),
        )
        // Tenant
        .route("/api/v1/tenants", post(handlers::manage::create_tenant))
        .route(
            "/api/v1/tenants/{tenantId}",
            get(handlers::manage::get_tenant),
        )
        .route(
            "/api/v1/tenants/{tenantId}",
            patch(handlers::manage::patch_tenant),
        )
        // Member
        .route(
            "/api/v1/tenants/{tenantId}/members",
            get(handlers::manage::list_members),
        )
        .route(
            "/api/v1/tenants/{tenantId}/members/{memberId}",
            patch(handlers::manage::patch_member),
        )
        .route(
            "/api/v1/tenants/{tenantId}/members/{memberId}",
            delete(handlers::manage::delete_member),
        )
        // Invitation
        .route(
            "/api/v1/tenants/{tenantId}/invitations",
            post(handlers::manage::create_invitation),
        )
        .route(
            "/api/v1/tenants/{tenantId}/invitations",
            get(handlers::manage::list_invitations),
        )
        .route(
            "/api/v1/tenants/{tenantId}/invitations/{invitationId}",
            delete(handlers::manage::cancel_invitation),
        )
        // /invite/{code}/accept — moved into `invite_routes` sub-router (rate-limited)
        // IdP Config
        .route(
            "/api/v1/tenants/{tenantId}/idp-configs",
            get(handlers::manage::list_idp_configs),
        )
        .route(
            "/api/v1/tenants/{tenantId}/idp-configs",
            post(handlers::manage::upsert_idp_config),
        )
        // M2M Client
        .route(
            "/api/v1/tenants/{tenantId}/m2m-clients",
            get(handlers::manage::list_m2m_clients),
        )
        .route(
            "/api/v1/tenants/{tenantId}/m2m-clients",
            post(handlers::manage::create_m2m_client),
        )
        ...
        .route(
            ...
            get(handlers::manage::list_passkeys),
        )
        .route(
            "/api/v1/users/{userId}/passkeys/{passkeyId}",
            delete(handlers::manage::delete_passkey),
        )
        // User management
        .route(
            "/api/v1/users/{userId}",
            patch(handlers::manage::patch_user),
        )
        ...
        .route(
            "/api/v1/tenants/{tenantId}/webhooks/{webhookId}",
            patch(handlers::webhook::patch_webhook),
        )
        .route(
            "/api/v1/tenants/{tenantId}/webhooks/{webhookId}",
        ...
        .route(
            ...
            get(handlers::webhook::webhook_deliveries),
        )
        // Audit
        ...
        .route(
            ...
            delete(handlers::admin::delete_device),
        )
        .route(
            "/api/v1/users/me/devices",
            delete(handlers::admin::delete_all_devices),
        )
        // Billing
        .route(
            "/api/v1/tenants/{tenantId}/billing",
            get(handlers::admin::get_billing),
        )
        .route(
            "/api/v1/tenants/{tenantId}/billing/subscription",
            post(handlers::admin::upsert_subscription),
        )
        // Policy
        .route(
            "/api/v1/tenants/{tenantId}/policies",
            get(handlers::admin::list_policies),
        )
        .route(
            "/api/v1/tenants/{tenantId}/policies",
            post(handlers::admin::create_policy),
        )
        .route(
            "/api/v1/tenants/{tenantId}/policies/evaluate",
            post(handlers::admin::evaluate_policy),
        )
        // GDPR
        .route(
            "/api/v1/users/me/data-export",
            post(handlers::admin::data_export),
        )
        .route(
            "/api/v1/users/{userId}/data",
            delete(handlers::admin::hard_delete_user),
        )
        // Admin (system)
        .route(
            "/api/v1/admin/tenants",
            get(handlers::admin::admin_list_tenants),
        )
        .route(
            "/api/v1/admin/users",
        ...
        .route(
            ...
            get(handlers::extra::admin_list_sessions),
        )
        .route(
            "/api/v1/admin/outbox/flush",
            post(handlers::admin::outbox_flush),
        )
        // SCIM 2.0
        ...
        .route(
            ...
            post(handlers::passkey_flow::register_start),
        )
        .route(
        ...
        .route(
            "/auth/sessions/{id}",
            delete(handlers::extra::revoke_session_by_id),
        )
        .route(
            "/auth/sessions/revoke-all",
            post(handlers::extra::revoke_all_sessions),
        )
        // Transfer ownership
        .route(
            "/api/v1/tenants/{tenantId}/transfer-ownership",
            post(handlers::extra::transfer_ownership),
        )
        // Switch account + select tenant
        .route(
            "/auth/switch-account",
            post(handlers::extra::switch_account),
        )
        ...
        .route(
            "/accounts/signout",
            post(handlers::accounts::signout_account),
        )
        .route(
            "/accounts/signout-all",
            post(handlers::accounts::signout_all),
        )
        // User export (admin)
        .route(
        ...
        .route(
            "/settings/security",
            get(handlers::extra::admin_members_page),
        )
        .route(
            "/settings/sessions",
            get(handlers::extra::admin_sessions_page),
        )
        ...
        .route(
            ...
            get(handlers::op::list_identities),
        )
        .route(
            "/api/v1/users/me/identities/{id}",
            delete(handlers::op::unlink_identity),
        )
        // Health + JWKS
