...
use volta_auth_core::store::{MfaStore, RecoveryCodeStore, SessionStore};
...
pub async fn mfa_challenge(State(state): State<AppState>, jar: CookieJar) -> Response {
    let session_id = match extract_session_id(&jar) {
        ...
        None => {
            let mut resp =
                axum::response::Redirect::to(&format!("{}/login", state.base_url)).into_response();
...
pub async fn totp_setup(
    ...
    MfaStore::upsert(&state.db, user_id, "totp", &secret)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
    // Immediately deactivate — will activate on verify
    MfaStore::deactivate(&state.db, user_id, "totp")
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
...
pub async fn totp_verify_setup(
    ...
    // Activate
    MfaStore::upsert(&state.db, user_id, "totp", &mfa.secret)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
    ...
    RecoveryCodeStore::replace_all(&state.db, user_id, &hashes)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
...
pub async fn totp_disable(
    ...
    MfaStore::deactivate(&state.db, user_id, "totp")
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
    RecoveryCodeStore::delete_all(&state.db, user_id)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
...
pub async fn mfa_status(
    ...
    let has_mfa = MfaStore::has_active(&state.db, user_id)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
    let recovery_count = if has_mfa {
        RecoveryCodeStore::count_unused(&state.db, user_id)
            .await
            .map_err(|e| ApiError::internal(&e.to_string()))?
    } else {
        0
...
pub async fn mfa_verify_login(
    ...
    let mfa = MfaStore::find(&state.db, user_id, "totp")
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?
    ...
    if !valid {
        ...
        let consumed = RecoveryCodeStore::consume(&state.db, user_id, &hash)
            .await
            .map_err(|e| ApiError::internal(&e.to_string()))?;
        if !consumed {
    ...
    SessionStore::mark_mfa_verified(&state.db, &session_id)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
...
async fn verify_session(state: &AppState, jar: &CookieJar) -> Result<SessionRecord, ApiError> {
    ...
    SessionStore::find(&state.db, &session_id)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?
        .ok_or_else(|| {
            ApiError::unauthorized(
