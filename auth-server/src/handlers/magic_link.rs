...
use volta_auth_core::store::{
    MagicLinkStore, MembershipStore, SessionStore, TenantStore, UserStore,
};
...
pub async fn send(
    ...
    MagicLinkStore::create(&state.db, &body.email, &token, 15)
        .await
...
pub async fn verify(
    ...
    let record = MagicLinkStore::consume(&state.db, &q.token)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?
        .ok_or_else(|| ApiError::bad_request("INVALID_TOKEN", "Invalid or expired magic link"))?;

    // Find or create user
    let user = match UserStore::find_by_email(&state.db, &record.email)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?
    {
        Some(u) => u,
        None => {
            ...
            UserStore::upsert(
                ...
                volta_auth_core::record::UserRecord {
                    ...
                    display_name: Some(
                        record.email.split('@').next().unwrap_or("user").to_string(),
                    ),
                    google_sub: None,
    ...
    let (tenant_id, tenant_slug, roles) = if let Some(t) = tenants.first() {
        let m = MembershipStore::find(&state.db, user.id, t.id)
            .await
            .map_err(|e| ApiError::internal(&e.to_string()))?;
        ...
        let t = TenantStore::create_personal(
            &state.db,
            user.id,
            &user.display_name.clone().unwrap_or(slug.clone()),
            &slug,
        )
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
