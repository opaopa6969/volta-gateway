...
pub async fn get_user(
    ...
    let _ = require_admin_with_headers(&s, &jar, Some(&headers)).await?;
    let user = UserStore::find_by_id(&s.db, id)
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?
...
pub async fn replace_user(
    ...
    let _ = require_admin_with_headers(&s, &jar, Some(&headers)).await?;
    UserStore::update_display_name(&s.db, id, b.display_name.as_deref().unwrap_or(""))
        .await
        .map_err(|e| ApiError::internal(&e.to_string()))?;
    let user = UserStore::find_by_id(&s.db, id)
        .await
        ...
        for op in ops {
            if op.get("path").and_then(|v| v.as_str()) == Some("displayName") {
                if let Some(val) = op.get("value").and_then(|v| v.as_str()) {
                    UserStore::update_display_name(&s.db, id, val)
                        .await
                        .map_err(|e| ApiError::internal(&e.to_string()))?;
                }
            }
