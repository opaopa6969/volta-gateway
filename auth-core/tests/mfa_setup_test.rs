...
async fn setup_stores_inactive_then_confirm_activates_and_issues_recovery_codes() {
    ...
    assert!(
        !MfaStore::has_active(&store, user).await.unwrap(),
        "must be inactive before confirm"
    );
    assert!(MfaStore::find_any(&store, user, "totp")
        .await
        .unwrap()
        .is_some());
    ...
    assert!(
        runtime::confirm_mfa_setup(&store, user, start.flow_id, "000000")
            .await
            .is_err()
    );
    ...
    let confirmed = runtime::confirm_mfa_setup(&store, user, start.flow_id, &code)
        .await
        .unwrap();
    assert!(
        MfaStore::has_active(&store, user).await.unwrap(),
        "active after confirm"
    );
    assert_eq!(confirmed.recovery_codes.len(), 10);

    // Recovery codes are stored hashed (count_unused = 10) and consumable.
    assert_eq!(
        RecoveryCodeStore::count_unused(&store, user).await.unwrap(),
        10
    );
    let first = &confirmed.recovery_codes[0];
    let hash = volta_auth_core::crypto::sha256_hex(first);
    assert!(RecoveryCodeStore::consume(&store, user, &hash)
        .await
        .unwrap());
    assert_eq!(
        RecoveryCodeStore::count_unused(&store, user).await.unwrap(),
...
async fn confirm_without_setup_is_not_found() {
    ...
    let user = make_user(&store).await;
    assert!(
        runtime::confirm_mfa_setup(&store, user, Uuid::new_v4(), "123456")
            .await
            .is_err()
    );
}
