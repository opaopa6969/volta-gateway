...
fn users(s: &PgStore) -> &(dyn UserStore + '_) {
    s
}
fn tenants(s: &PgStore) -> &(dyn TenantStore + '_) {
    s
}
fn memberships(s: &PgStore) -> &(dyn MembershipStore + '_) {
    s
}
fn invitations(s: &PgStore) -> &(dyn InvitationStore + '_) {
    s
}
fn flows(s: &PgStore) -> &(dyn FlowPersistence + '_) {
    s
}
...
async fn user_upsert_and_find() {
    ...
    // Find by google_sub
    let found = users(&store)
        .find_by_google_sub("google-alice")
        .await
        .unwrap()
        .unwrap();
    assert_eq!(found.id, created.id);
    ...
    // Update display name
    users(&store)
        .update_display_name(created.id, "Alice Updated")
        .await
        .unwrap();
    let found = users(&store).find_by_id(created.id).await.unwrap().unwrap();
...
async fn tenant_crud() {
    ...
    let tenant = tenants(&store)
        .create(TenantRecord {
            id: Uuid::new_v4(),
            name: "Acme Corp".into(),
            slug: "acme".into(),
            email_domain: Some("acme.com".into()),
            auto_join: false,
...
async fn tenant_create_personal_with_membership() {
    ...
    let tenant = tenants(&store)
        .create_personal(user.id, "Personal", "personal-user")
        .await
        .unwrap();
...
async fn membership_crud() {
    ...
    let tenant = tenants(&store)
        .create_personal(owner.id, "Team", "mem-team")
        .await
        .unwrap();
    ...
    let found = memberships(&store)
        .find(member.id, tenant.id)
        .await
        .unwrap()
        .unwrap();
    ...
    memberships(&store)
        .update_role(found.id, "ADMIN")
        .await
        .unwrap();
    let updated = memberships(&store)
        .find(member.id, tenant.id)
        .await
        .unwrap()
        .unwrap();
...
async fn invitation_crud_and_accept() {
    ...
    let tenant = tenants(&store)
        .create_personal(owner.id, "Inv Team", "inv-team")
        .await
        .unwrap();

    invitations(&store)
        .create(InvitationRecord {
            id: Uuid::new_v4(),
            tenant_id: tenant.id,
            code: "inv-code-123".into(),
    ...
    let found = invitations(&store)
        .find_by_code("inv-code-123")
        .await
        .unwrap()
        .unwrap();
    ...
    // Accept
    invitations(&store)
        .accept("inv-code-123", invitee.id)
        .await
        .unwrap();
...
async fn invitation_cancel() {
    ...
    let tenant = tenants(&store)
        .create_personal(owner.id, "Cancel Team", "cancel-team")
        .await
        .unwrap();
...
async fn flow_lifecycle() {
    ...
    // Record transition
    flows(&store)
        .record_transition(flow_id, Some("Init"), "Redirected", "auto:OidcInit", None)
        .await
        .unwrap();

    // Update state (optimistic lock: version 0 → 1)
    flows(&store)
        .update_state(flow_id, "Redirected", 1)
        .await
        .unwrap();
    let found = flows(&store).find(flow_id).await.unwrap().unwrap();
    ...
    flows(&store)
        .complete(flow_id, "Complete", Some(summary))
        .await
        .unwrap();
    let found = flows(&store).find(flow_id).await.unwrap().unwrap();
...
fn device_grants(s: &PgStore) -> &(dyn DeviceGrantStore + '_) {
    s
}

async fn setup_device_pool() -> (
    PgPool,
    testcontainers::ContainerAsync<testcontainers_modules::postgres::Postgres>,
) {
    ...
    sqlx::raw_sql(include_str!(
        "../migrations/028_create_device_authorization_grants.sql"
    ))
    .execute(&pool)
    .await
    .unwrap();
...
async fn device_grant_full_lifecycle() {
    ...
    device_grants(&store)
        .create(new_grant("device-code-1", "WDJB-MJHT", 0, 600))
        .await
        .unwrap();

    // Visible to the approval page.
    let pending = device_grants(&store)
        .find_pending_by_user_code("WDJB-MJHT")
        .await
        .unwrap();
    ...
    assert_eq!(
        device_grants(&store).poll(&hash).await.unwrap(),
        DevicePollOutcome::Pending
    );

    // User approves.
    let dec = device_grants(&store)
        .decide("WDJB-MJHT", true, user_id, tenant_id)
        .await
        .unwrap();
...
async fn device_grant_deny_and_slow_down_and_expiry() {
    ...
    // Deny → access_denied.
    device_grants(&store)
        .create(new_grant("dc-deny", "AAAA-BBBB", 0, 600))
        .await
        .unwrap();
    device_grants(&store)
        .decide("AAAA-BBBB", false, uid, tid)
        .await
        .unwrap();
    let h_deny = volta_auth_core::crypto::sha256_hex("dc-deny");
    assert_eq!(
        device_grants(&store).poll(&h_deny).await.unwrap(),
        DevicePollOutcome::Denied
    );

    // slow_down: interval 5, two quick polls → second is throttled.
    device_grants(&store)
        .create(new_grant("dc-slow", "CCCC-DDDD", 5, 600))
        .await
        .unwrap();
    let h_slow = volta_auth_core::crypto::sha256_hex("dc-slow");
    assert_eq!(
        device_grants(&store).poll(&h_slow).await.unwrap(),
        DevicePollOutcome::Pending
    );
    assert_eq!(
        device_grants(&store).poll(&h_slow).await.unwrap(),
    ...
    device_grants(&store)
        .create(new_grant("dc-exp", "EEEE-FFFF", 0, -1))
        .await
        .unwrap();
    let h_exp = volta_auth_core::crypto::sha256_hex("dc-exp");
    assert_eq!(
        device_grants(&store).poll(&h_exp).await.unwrap(),
...
async fn setup_oauth_pool() -> (
    ...
    sqlx::raw_sql(include_str!("../migrations/029_create_oauth_provider.sql"))
        ...
        .unwrap();
    sqlx::raw_sql(include_str!(
        "../migrations/031_oauth_client_logout_uris.sql"
    ))
    .execute(&pool)
    .await
    .unwrap();
...
fn oauth_client(s: &PgStore) -> &(dyn OAuthClientStore + '_) {
    s
}
fn authz(s: &PgStore) -> &(dyn AuthzCodeStore + '_) {
    s
}
fn refresh(s: &PgStore) -> &(dyn RefreshTokenStore + '_) {
    s
}
fn consent(s: &PgStore) -> &(dyn OAuthConsentStore + '_) {
    s
}
...
async fn oauth_client_and_consent() {
    ...
    let store = PgStore::new(pool);
    oauth_client(&store)
        .create_client(OAuthClientRecord {
            id: Uuid::new_v4(),
            ...
            backchannel_logout_uri: Some("https://rp/bclogout".into()),
            frontchannel_logout_uri: None,
            created_at: Utc::now(),
        })
        .await
        .unwrap();
    let c = oauth_client(&store)
        .find_client("cli-1")
        .await
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        c.backchannel_logout_uri.as_deref(),
        Some("https://rp/bclogout")
    );

    let uid = Uuid::new_v4();
    assert!(!consent(&store)
        .has_consent(uid, "cli-1", "openid email")
        .await
        .unwrap());
    consent(&store)
        .grant_consent(uid, "cli-1", "openid email profile")
        .await
        .unwrap();
    ...
    assert!(!consent(&store)
        .has_consent(uid, "cli-1", "offline_access")
        .await
        .unwrap());
}
...
async fn refresh_rotation_and_reuse_detection() {
    ...
    assert!(matches!(
        refresh(&store).rotate_refresh(&h1).await.unwrap(),
    ...
    assert_eq!(
        refresh(&store).rotate_refresh(&h2).await.unwrap(),
        RefreshOutcome::Reused
    );
    // unknown token → NotFound
    assert_eq!(
        refresh(&store).rotate_refresh("nope").await.unwrap(),
...
async fn risk_device_new_then_known() {
    let (pool, _c) = setup_pool().await;
    sqlx::raw_sql(include_str!(
        "../migrations/030_create_risk_known_devices.sql"
    ))
    .execute(&pool)
    .await
    .unwrap();
    ...
    assert!(!RiskDeviceStore::check_and_record(&store, user, &dev)
        .await
        .unwrap());
    ...
    assert!(!RiskDeviceStore::check_and_record(&store, user, &dev2)
        .await
        .unwrap());
    // and the same device for a different user is new
    assert!(
        !RiskDeviceStore::check_and_record(&store, Uuid::new_v4(), &dev)
            .await
            .unwrap()
    );
}
...
fn identities(s: &PgStore) -> &(dyn UserIdentityStore + '_) {
    s
}
...
async fn account_linking_multiple_providers() {
    ...
    let found = identities(&store)
        .find_by_subject("github", "h-1")
        .await
        .unwrap()
        .unwrap();
    assert_eq!(found.user_id, user);
    assert!(identities(&store)
        .find_by_subject("github", "nope")
        .await
        .unwrap()
        .is_none());
    ...
    let gh = identities(&store)
        .find_by_subject("github", "h-1")
        .await
        .unwrap()
        .unwrap();
    assert!(!identities(&store)
        .unlink(Uuid::new_v4(), gh.id)
        .await
        .unwrap());
