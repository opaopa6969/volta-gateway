...
impl FlowState for InviteState {
    fn is_terminal(&self) -> bool {
        matches!(self, Self::Complete | Self::TerminalError | Self::Expired)
    }
    fn is_initial(&self) -> bool {
        matches!(self, Self::ConsentShown)
    }
    fn all_states() -> &'static [Self] {
        &[
            Self::ConsentShown,
            Self::AccountSwitching,
            Self::Accepted,
            Self::Complete,
            Self::TerminalError,
            Self::Expired,
    ...
    fn decide(&self, ctx: &FlowContext) -> String {
        match ctx.find::<InviteData>() {
            Some(data) => match &data.invite_email {
                Some(email) if email.eq_ignore_ascii_case(&data.current_user_email) => {
                    "match".into()
                }
                _ => "switch".into(),
            },
...
pub fn build_invite_flow() -> Arc<FlowDefinition<InviteState>> {
    ...
    Arc::new(
        Builder::new("invite")
            ...
            .from(ConsentShown)
            .branch(EmailMatchBranch)
            .to(Accepted, "match")
            .to(AccountSwitching, "switch")
            .end_branch()
            .from(AccountSwitching)
            .external(Accepted, AcceptGuard)
            .from(Accepted)
            .auto(Complete, InviteCompleteProcessor)
...
mod tests {
    ...
    fn invite_accept_after_switching() {
        ...
        let accept: Vec<(TypeId, Box<dyn CloneAny>)> = vec![(
            TypeId::of::<InviteAcceptance>(),
            Box::new(InviteAcceptance { accepted: true }),
        )];
